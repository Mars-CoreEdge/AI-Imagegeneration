...
async function deletePost(id, onDelete) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      onDelete(id) // Call the callback to remove from UI
    } else {
  ...
  return (
    <article className="group relative overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
      ...
      <div className="pointer-events-none absolute inset-0 flex items-end justify-between bg-gradient-to-b from-transparent to-black/60 p-3 opacity-0 transition-opacity duration-200 group-hover:pointer-events-auto group-hover:opacity-100">
        <div className="max-w-[calc(100%-96px)]">
        ...
        <div className="flex gap-2">
          ...
          <button
            className="pointer-events-auto inline-flex h-9 w-9 items-center justify-center rounded-lg border border-red-700 bg-red-800 text-white hover:bg-red-700"
            aria-label="Delete post"
            onClick={() => deletePost(image.id, onDelete)}
          >
            🗑
          </button>
        </div>
